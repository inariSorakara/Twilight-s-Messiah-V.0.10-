...
var monster_type:String = "Boss"

var monster_damage:int = 40

var monster_memoria_loot = 60

var memoria_cost:int = 20

#region DIALOGUES

var enemy_encounter: String = "You have encountered a " + monster_type + " enemy!"

var flavor_encounter_text:String = "...Good Luck."
...
func trigger_event():
	...
	state_owner.event_presenter("res://Assests/Art/Events/Iron/Iron_Encounter.png")
	
	var encounter_text:Array = []
	
	encounter_text.append(enemy_encounter)
	
	encounter_text.append(flavor_encounter_text)
	...
	if (current_player.current_health - monster_damage) > 0:
		...
		current_player.current_memorias += monster_memoria_loot
		current_player.total_memorias += monster_memoria_loot
	else:
		current_player.current_health -= monster_damage
		encounter_text.append_array(player_damaged)
